...
export default function LocationMapCard() {
  ...
  return (
    <>
      <div className="w-full h-[360px] sm:h-[400px] lg:h-[460px] rounded-3xl overflow-hidden border border-amber-200/90 shadow-2xl relative bg-slate-100 group flex flex-col">
        {/* Top Header Bar (No overlapping on mobile) */}
        <div className="absolute top-3 inset-x-3 sm:top-4 sm:inset-x-4 z-20 flex items-center justify-between gap-2 pointer-events-none">
          {/* Top Floating Badge (Visible on sm+ screens to avoid mobile header collision) */}
          <div className="hidden sm:flex px-3.5 py-1.5 rounded-full bg-white/95 border border-amber-300 text-amber-900 text-xs font-bold backdrop-blur-md items-center gap-2 shadow-md">
            <MapPin className="w-3.5 h-3.5 text-[#D49E17] animate-pulse" />
            <span>Saffron City — Main GT Road, Rawat</span>
          </div>

          {/* View Switcher Controls */}
          <div className="pointer-events-auto flex items-center gap-1.5 p-1 bg-white/95 rounded-2xl border border-amber-300/80 shadow-lg backdrop-blur-md ml-auto">
            <button
              type="button"
        ...
        {/* Bottom Action & Status Bar (Responsive, no overlap) */}
        <div className="absolute bottom-3 inset-x-3 sm:bottom-4 sm:inset-x-4 z-20 flex flex-wrap items-center justify-between gap-2 pointer-events-none">
          <div className="pointer-events-auto flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-bold transition-colors flex items-center gap-1.5 shadow-lg cursor-pointer"
            >
      ...
      <DownloadLeadModal
        ...
        downloadUrl="/images/imgi_87_LOCATION.webp"
